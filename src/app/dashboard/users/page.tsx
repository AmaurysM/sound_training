...
'use client';
import React, { useEffect, startTransition, useRef } from 'react';
import { useRouter } from 'next/navigation';
import { Search, UserPlus, Archive, ArchiveRestore, Users, LogOut, User as UserIcon, BookOpen, Settings, ChevronDown, Menu, X, Mail, CheckCircle, Clock, Trash2, ArrowLeft } from 'lucide-react';
...
export default function ViewStudents() {
    ...
    const [searchRoleDropdownOpen, setSearchRoleDropdownOpen] = React.useState(false);
                                            ...
                                            <ul className="absolute z-50 mt-1 w-full bg-white border border-gray-300 rounded shadow-lg max-h-60 overflow-y-auto">
                                                {roleOptions.map((opt) => (
                                                    <li
                                                        key={opt.value}
                                                        onClick={() => {
                                                            setNewUserForm({ ...newUserForm, role: opt.value });
                                                            setSearchRoleDropdownOpen(false);
                                                        }}
                                ...
                                {filteredUsers.map((user) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3">
                                                {user.archived ? (
                                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium text-gray-700 bg-gray-100 rounded-full">
                                    ...
                                    <div className="flex justify-between items-start gap-4">
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-2">
                                                <div className="text-sm font-medium truncate">{user.name}</div>
                                                {user.archived && (
                                                    <span className="inline-flex px-2.5 py-1 text-xs font-medium text-gray-700 bg-gray-200 rounded-full shrink-0">
                                                        Archived
                                                    </span>
                                                )}
                                    ...
                                    <div className="flex justify-between items-start gap-3 mb-2">
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-1">
                                                <div className="text-sm font-medium truncate">{user.name}</div>
                                                {user.archived && (
                                                    <span className="inline-flex px-2 py-0.5 text-xs font-medium text-gray-700 bg-gray-200 rounded-full">
                                                        Archived
                                                    </span>
                                                )}
                                            </div>
                                        ...
                                        <div className="flex flex-col items-end gap-1 shrink-0">
                                            ...
                                            {user.archived ? (
                                                <span className="inline-flex items-center gap-1 px-2 py-0.5 text-xs font-medium text-gray-700 bg-gray-100 rounded-full">
