// src/app/dashboard/users/[userId]/modules/[moduleId]/submodules/page.tsx
...
export default function SubmodulesPage() {
    ...
    useEffect(() => {
        ...
        const initializeData = async () => {
            try {
                ...
                // Fetch current user from context if not available
                if (!currentUser) {
                    await fetchCurrentUser();
                }

                // // Fetch viewed user and modules from context
                // if (!viewedUser || viewedUser._id !== userId) {
                //     await fetchViewedUserAndModules(userId as string);
                // }
                ...
                const moduleRes = await fetch(`/api/users/${userId}/modules/${moduleId}`, {
                    signal: abortController.signal
                });

                if (moduleRes.ok) {
                    const moduleResponse = await moduleRes.json();
                    setModuleDetails({ archived: moduleResponse.data?.archived || false });
                }
            } catch (err) {
    ...
    const canEditFields = () => {
        ...
        if (viewedUser?.archived || moduleDetails?.archived) return false;
    ...
    const toggleField = async (submoduleId: string, field: "ojt" | "practical", current: boolean) => {
        if (!canEditFields()) {
            if (viewedUser?.archived || moduleDetails?.archived) {
        ...
        try {
            ...
            // ✅ REMOVED: await fetchViewedUserAndModules(userId as string);
    ...
    return (
        <div className="min-h-screen bg-slate-50 p-3 sm:p-4 md:p-6">
            <div className="max-w-7xl mx-auto">
                {/* Archived Banner */}
                {(viewedUser?.archived || moduleDetails?.archived) && (
                ...
                <div className="flex flex-row items-center justify-between gap-2 mb-4">
                    <button
                        onClick={() => router.push(`/dashboard/users/${userId}/modules/${moduleId}`)}
                                                ...
                                                <tr key={submodule._id} className="hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <td className="p-2 md:p-3">
                                                        <button
                                                            ...
                                                            title={canEditFields() ? "Click to toggle OJT completion" : (viewedUser?.archived || moduleDetails?.archived) ? "Cannot edit - archived" : "You don't have permission to edit"}
                                                        >
                                                    ...
                                                    <td className="p-2 md:p-3">
                                                        {submodule.tSubmodule?.requiresPractical ? (
                                                            <button
                                                                ...
                                                                title={canEditFields() ? "Click to toggle practical completion" : (viewedUser?.archived || moduleDetails?.archived) ? "Cannot edit - archived" : "You don't have permission to edit"}
                                                            >
                                        ...
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            {sig ? (
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {userHasSigned && !(viewedUser?.archived || moduleDetails?.archived) && (
                                                        <button
