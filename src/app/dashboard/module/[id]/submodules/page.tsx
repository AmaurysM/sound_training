...
import { ISignature, ITrainingSubModule } from "@/models/types";

interface SortConfig {
    key: keyof ITrainingSubModule | 'signatureStatus';
    direction: 'asc' | 'desc';
}
...
export default function SubmodulesPage() {
    ...
    const [signOffModal, setSignOffModal] = useState<{ open: boolean; submodule: ITrainingSubModule | null }>({
        open: false,
        submodule: null,
    });
    ...
    useEffect(() => {
        const fetchSubmodules = async () => {
            try {
                setLoading(true);
                const res = await fetch(`/api/trainings/${id}/submodules`);
                if (!res.ok) throw new Error("Failed to fetch submodules");
                const data: ITrainingSubModule[] = await res.json();
                setSubmodules(data || []);
            } catch (err) {
                console.error(err);
            } finally {
    ...
    const hasAllSignatures = (submodule: ITrainingSubModule) => {
    ...
    const filteredAndSortedSubmodules = useMemo(() => {
        ...
        if (practicalFilter !== null) {
            filtered = filtered.filter(s => s.requiresPractical === practicalFilter);
        }
        ...
        if (sortConfig.key) {
            filtered.sort((a, b) => {
                ...
                let aValue: any = a[sortConfig.key as keyof ITrainingSubModule];
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                let bValue: any = b[sortConfig.key as keyof ITrainingSubModule];
    ...
    const getSignatureStatus = (submodule: ITrainingSubModule) => {
    ...
    const toggleField = async (submoduleId: string, field: "ojt" | "practical", current: boolean) => {
        try {
            const res = await fetch(`/api/submodule/${submoduleId}`, {
                method: "PATCH",
            ...
            setSubmodules(prev =>
                prev.map(s => (s._id === submoduleId ? { ...s, ...updated } : s))
            );
        } catch (err) {
            console.error(err);
    ...
    const openSignOffModal = async (submodule: ITrainingSubModule) => {
        try {
            const res = await fetch(`/api/submodule/${submodule._id}`);
            if (!res.ok) throw new Error("Failed to fetch submodule details");
            const detailed = await res.json();
            setSignOffModal({ open: true, submodule: detailed });
        } catch (err) {
            console.error(err);
    ...
    const addSignature = async (role: "Coordinator" | "Trainer" | "Trainee") => {
        ...
        try {
            ...
            const existingSig = currentSigs.find(sig => sig.role === role && sig.userId === currentUser._id);
            if (existingSig) {
            ...
            const res = await fetch(`/api/submodule/${signOffModal.submodule._id}`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
            ...
            const updated = await res.json();
            setSubmodules(prev =>
                prev.map(s => (s._id === signOffModal.submodule?._id ? { ...s, ...updated } : s))
            );
            setSignOffModal({ open: true, submodule: updated });
        } catch (err) {
            console.error(err);
    ...
    const hasUserSignedRole = (role: "Coordinator" | "Trainer" | "Trainee") => {
        ...
        return signOffModal.submodule.signatures?.some(sig =>
            sig.role === role && sig.userId === currentUser._id
        );
    };
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    <thead className="bg-slate-50 border-b border-slate-200">
                                        <tr>
                                            <SortableHeader sortKey="code" className="w-20">Code</SortableHeader>
                                            <SortableHeader sortKey="title">Title</SortableHeader>
                                            <SortableHeader sortKey="requiresPractical" className="w-32">Type</SortableHeader>
                                        ...
                                        {filteredAndSortedSubmodules.map((submodule) => {
                                            ...
                                            return (
                                                <tr key={submodule._id} className="hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <td className="p-2 md:p-3">
                                                        <div className="font-medium text-slate-900 text-sm">{submodule.title}</div>
                        ...
                        <div className="p-4 sm:p-6 border-b border-slate-200 flex items-center justify-between">
                            <div className="flex-1 min-w-0">
                                <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-slate-900 truncate">
                                    Sign Off: {signOffModal.submodule.title}
                                </h2>
                                    ...
                                    <div key={role} className="border border-slate-200 rounded-lg p-3 sm:p-4">
                                        ...
                                        {sig ? (
                                            <div className="text-sm text-slate-600">
                                                ...
                                                <p className="text-xs">Signed: {new Date(sig.signedAt).toLocaleString()}</p>
                                            </div>
                                        ) : (
                                            <div className="text-sm text-slate-600">
                                                {canSign ? (
